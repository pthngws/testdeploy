...
import com.group4.services.RateService;
...
//    }
    ...
    public String getRateList(Model model) {
        List<RateEntity> rates = rateService.getAllRates(); // Lấy tất cả đánh giá
    ...
    public String getRateResponseForm(@PathVariable("rateID") Long rateID, Model model) {
        RateEntity rate = rateService.getRateById(rateID); // Lấy đánh giá dựa trên rateID
    ...
    public String submitResponse(@PathVariable("rateID") Long rateID, @RequestParam("response") String response) {
        RateEntity rate = rateService.getRateById(rateID);
        rate.setResponse(response); // Lưu phản hồi vào đánh giá
        rateService.saveRate(rate); // Lưu lại vào cơ sở dữ liệu
    ...
    public String viewResponse(@PathVariable("rateID") Long rateID, Model model) {
        RateEntity rate = rateService.getRateById(rateID); // Lấy thông tin đánh giá
        model.addAttribute("rate", rate);
